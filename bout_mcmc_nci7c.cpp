...
// y2 weibull
//same as 6 but with power link instead of log link, power of -2, sqrt(y1)
...
arma::mat sample_b(arma::mat y1, arma::mat y2,arma::vec muy1,arma::mat muy2,arma::mat currentb, 
  ...
  for(int i=0;i<n;i++){
    ...
    lacceptprob = log_qb(y1.row(i),y2.row(i),exp(muy1prop),exp(muy2prop),lambda,phi,propb,Sigmab) - 
...
List mcmc_2part_nci7c(List data, 
  ...
  x1x2y1 = arma::join_rows(Za,sqrt(y1.col(0)));//arma::join_rows(Za,sqrt(y1.col(0)));//x1x2;//arma::join_rows(intercept,currentx2);
  x1x2y2 = arma::join_rows(Za,sqrt(y1.col(1)));//arma::join_rows(Za,sqrt(y1.col(1)));//x1x2;//arma::join_rows(intercept,currentx2);
