...
// re in mean of y1 and in mean of y2, 2-dim normal
//y2 lognormal
...
//// [[Rcpp::export]]
double log_qgamma(arma::vec cx, arma::mat vx, double lambda, arma::vec mu1, arma::mat y1, arma::vec gam){
  double out = 0.0;
  int n = y1.n_rows;
  int k = y1.n_cols;
  for(int i=0;i<n;i++){
...
arma::vec sample_gamma(arma::vec cx, arma::mat vx, double lambda, arma::vec mu1, 
  ...
  lacceptprob = log_qgamma(cx,vx,lambda,mu1prop,y1,proposal)-log_qgamma(cx,vx,lambda,mu1,y1,gamma);
...
// [[Rcpp::export]]
List mcmc_2part_nci2c(List data, 
                      List init, 
                      List priors, 
                      const int nreps, 
                      const int burn=1000){
  ...
  for(int i=0;i<nreps;i++){
    ...
    currentlambda = sample_lambda(y1,exp(currentlmux1),currentlambda,a0l,b0l,propl1,propl2);
