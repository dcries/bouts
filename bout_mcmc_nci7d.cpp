...
// [[Rcpp::export]]
List mcmc_2part_nci7d(List data, 
                      List init, 
                      List priors, 
                      const int nreps, 
                      const int burn=1000){
  ...
  arma::mat betax_var = arma::zeros(nb,nb);
 ...
 // arma::mat alpha_var = arma::zeros(mu0a.size(),mu0a.size());
  ...
  x1x2y1 = Za;//arma::join_rows(Za,log(y1.col(0)));//arma::join_rows(Za,sqrt(y1.col(0)));//x1x2;//arma::join_rows(intercept,currentx2);
  x1x2y2 = Za;//arma::join_rows(Za,log(y1.col(1)));//arma::join_rows(Za,sqrt(y1.col(1)));//x1x2;//arma::join_rows(intercept,currentx2);
