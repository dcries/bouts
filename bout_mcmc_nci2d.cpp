...
// model made at NCI with no random effects for lambda in gpois, 
// gamma random effects for y1
// normal random error for y2 and normal re for person indpt of y1 re
...
//// [[Rcpp::export]]
double sample_sigma2(double a0, double b0, arma::vec xp){
  double out;
  double a = xp.size()/2.0 + a0;
  double b = b0 + 0.5*arma::accu(pow(xp,2.0));
...
double log_qb(arma::rowvec y2, arma::vec muy, double p,
              double currentb, double sigma2y, double sigma2b){
  int n = y2.size();
  double ll=0.0;
  ...
  for(int i=0;i<n;i++){
    ...
    if(y2[i]==0){
      ...
      ll += log(p) + R::dlnorm(y2[i],muy[i],sqrt(sigma2y),true);
    }
  }
...
arma::vec sample_b(arma::mat y2, arma::mat muy2,arma::vec currentb, 
                   double sigma2y, double sigma2b, arma::vec betay,
                   arma::vec p, arma::mat Zby1,arma::mat Zby2,arma::vec Sigma_1){
  //std::cout << "a\n";
  ...
  for(int i=0;i<n;i++){
    ...
    //std::cout << "b\n";
    propb = R::rnorm(currentb[i],sqrt(pow(2.4,2)*Sigma_1[i]));
    //std::cout << "c\n";
    for(int j=0;j<k;j++){
      muy2prop[0] += Zby1(i,j)*betay[j];
      muy2prop[1] += Zby1(i,j)*betay[j];
    }
    muy2prop[0] += propb;
    muy2prop[1] += propb;
    ...
    lacceptprob = log_qb(y2.row(i),muy2prop,p[i],propb,sigma2y,sigma2b) - 
      log_qb(y2.row(i),muy2.row(i).t(),p[i],currentb[i],sigma2y,sigma2b);
    //std::cout << "e\n";
...
// [[Rcpp::export]]
List mcmc_2part_nci2d(List data, 
                     List init, 
                     List priors, 
                     const int nreps, 
                     const int burn=1000){
  ...
  //starting values
  arma::vec currentb = as<arma::vec>(init["currentb3"]);
  ...
  //arma::mat b;
  arma::vec bsub = arma::join_cols(subset2(currentb,indy1),subset2(currentb,indy2));
  ...
  for(int i=0;i<nreps;i++){
    ...
    currentbetay = sample_beta_2(x1x2p,mu0y2,V0y2,currentsigma2y,y2sub,bsub);
    currentlmuy = calc_lmu(x1x2y1,currentbetay,currentb);
    currentlmuy2 = calc_lmu(x1x2y2,currentbetay,currentb);
    ...
    currentb = sample_b(y2,arma::join_rows(currentlmuy,currentlmuy2),currentb,currentsigma2y,
                        currentsigma2b,currentbetay,currentp,
                        x1x2y1,x1x2y2,b_var);
      // 
    if((i>99)&&(i<burn)&&(i%20==0)){
      for(int j=0;j<n;j++){
        b_var[j] = var(b1.col(j).rows(0,i-1));
      }
    }
    
    bsub = arma::join_cols(subset2(currentb,indy1),subset2(currentb,indy2));
    
    //std::cout << "7\n";
    
    currentsigma2y = sample_sigma2(a0y,b0y,y2sub1,y2sub2,x1x2p1,x1x2p2,currentbetay,bsub);
    currentsigma2b = sample_sigma2(a0x,b0x,currentb);
    ...
    sigma2b[i] = currentsigma2b;//(currentSigmab.diag()).t();
    ...
    if(i >= burn){
      ind_dic[i-burn] = calc_ind_dic(y1,y2,currentx1,exp(currentlmux1),currentlambda,
                                     currenteta,currentsigma2y,currentbetay,
                                     x1x2y1,x1x2y2,currentb,currentsigma2b);
    }
