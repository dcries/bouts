...
Za <- model.matrix(~age+as.factor(gender)+as.factor(smoke)+(education)+(black)+as.factor(hispanic),data=Za)
...
names(Z) <- c("int","age","gender","smoke","education","black","hispanic")
y = y2[y2>0]
...
assessln <- pp_assess(mcmc,data$Zb,1000,"9b",data$y1,data$y2,weights,burn=0)

save(mcmc,file="boutsmcmc_nhanes.RData")
save(assessln,file="assessmcmc_nhanes.RData")