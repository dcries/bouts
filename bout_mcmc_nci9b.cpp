...
double sample_lambda(arma::mat y1, arma::mat y2, arma::vec x1, arma::mat muy2,
  ...
  double out = lambda;
  double proposal = R::runif(-.1,.1);//rbeta(propl1,propl2);
  double lacceptprob;
  int n = x1.size();
  arma::vec pprop(n);
...
double log_qb(arma::rowvec y1, arma::rowvec y2, double muy, arma::vec muy2, double lambdau,
  ...
  for(int i=0;i<n;i++){
    ...
    //std::cout << dgenpois(y1[i],muy,lambdau,true) << " " << log(p0) + R::dweibull(y2[i],phi,muy2[i],true) << "\n";
  }
...
arma::mat sample_b(arma::mat y1, arma::mat y2,arma::vec muy1,arma::mat muy2,arma::mat currentb, 
  ...
  for(int i=0;i<n;i++){
    ...
    lacceptprob = log_qb(y1.row(i),y2.row(i),exp(muy1prop),(muy2prop),lambda,phi,propb,Sigmab) - 
...
// [[Rcpp::export]]
List mcmc_2part_nci9b(List data, 
                     List init, 
                     List priors, 
                     const int nreps, 
                     const int burn=1000,
