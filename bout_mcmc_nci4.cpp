...
// same as model 1 except with an independent normal re for lambda
...
double log_qb(arma::rowvec y1, double muy, double lambdau,
              double currentb, double sigma2b, double M){
  int n = y1.size();
  double ll=0.0;
  ...
  ll += R::dnorm4(currentb,0,sqrt(sigma2b),true);
  //}
...
arma::vec sample_b(arma::mat y1, arma::vec mux1,arma::vec currentb, double sigma2b, arma::vec gamma,
                   double lambda, arma::vec M, double Sigma_1){
  //std::cout << "a\n";
  ...
  for(int i=0;i<n;i++){
    mux1prop = 0.0;
    //std::cout << "b\n";
    propb = R::rnorm(currentb[i],sqrt(pow(2.4,2)*Sigma_1));
    //std::cout << "c\n";

    lambdauprop = lambda + propb;
    //std::cout << "d\n";
    
    lacceptprob = log_qb(y1.row(i),mux1[i],lambdauprop,propb,sigma2b,M[i]) - 
      log_qb(y1.row(i),mux1[i],lambda+currentb[i],currentb[i],sigma2b,M[i]);
    //std::cout << "e\n";
...
double calc_ind_dic(arma::mat y1, arma::mat y2, arma::vec x1,
                    ...
                    arma::vec b, double sigma2b, arma::vec m){
  int n = y1.n_rows;
  int k = y1.n_cols;
  int p = yZ1.n_cols;
  double ll = 0.0;
  ...
  for(int i=0;i<n;i++){
    ...
    prob = 1-dgenpois(0,x1[i],lambda+b[i],false,m[i]);
    for(int j=0;j<k;j++){
      ll += dgenpois(y1(i,j),x1[i],lambda +b[i],true,m[i]);
      if(y2(i,j)==0){
        ll += log(1-prob);
      }
      else{
    ...
    //ll += R::dnorm4(b[i],0,sqrt(sigma2b),true);
  }
  out = ll / (n*k);
  return out;
}
...
// [[Rcpp::export]]
List mcmc_2part_nci4(List data, 
                     List init, 
                     List priors, 
                     const int nreps, 
                     const int burn=1000){
  ...
  //starting values
  //arma::vec currentu = as<arma::mat>(init["currentu"]);
  ...
  for(int i=0;i<nreps;i++){
    ...
    currentlmuy2 = calc_lmu(x1x2y1,currentbetay,arma::zeros(n));
    
    //std::cout << "6b\n";
    
    //std::cout << "6c\n";
    
    currentgamma = sample_gamma(mu0x1,V0x1,currenteta,exp(currentlmux1),currentx1,Za,currentgamma,gamma_var,arma::zeros(n));
    
    currentlmux1 = calc_lmu(Za,currentgamma,arma::zeros(n));
    
    currentlambda = sample_lambda(y1,currentx1,currentu,currentlambda,currentm);
    ...
    currentu = sample_b(y1,exp(currentlmux1),currentu,currentsigma2b,currentgamma,
                        currentlambda,currentm,b_var);
    // 
    // if((i>99)&&(i<burn)&&(i%20==0)){
    //   for(int j=0;j<n;j++){
    // 
    //     b_var = var(b2.rows(0,i-1));
    //   }
    // }
    ...
    currentsigma2b = sample_sigma2(a0x,b0x,currentu);
    ...
    if(i >= burn){
      ind_dic[i-burn] = calc_ind_dic(y1,y2,currentx1,exp(currentlmux1),currentlambda,
                                     ...
                                     currentu,currentsigma2b,currentm);
    }
  ...
  mean_dic = calc_ind_dic(y1,y2,trans(mean(latentx1,0)),trans(mean(mux1,0)),mean(lambda),
                          ...
                          trans(mean(b2,0)),mean(sigma2b),trans(mean(m,0)));
