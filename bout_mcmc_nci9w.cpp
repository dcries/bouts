...
double loglik_yi(arma::rowvec y1,arma::rowvec y2, arma::vec b, arma::rowvec Z, arma::vec gamma,arma::vec beta, double lambda,double sigma2y,
                 arma::mat Sigmab, double weight, int m=1000){
  double ll = 0.0;
  int k = y1.size();
  int p = gamma.size();
  double muy1;
  double muy2;
  double prob;
  arma::vec eval = arma::zeros(m);
  arma::mat newbm(1,2);
  arma::vec newb(2);
  
  for(int i=0;i<m;i++){
    newbm.row(0) = mvrnormArma(1,b,Sigmab);
    newb[0] = newbm(0,0);
    newb[1] = newbm(0,1);
    muy1=muy2=0.0;
    for(int j=0;j<p;j++){
    ...
    prob = (1-dgenpois(0,exp(muy1),lambda,false));
    
    for(int j=0;j<k;j++){
      eval[i] += dgenpois(y1[j],exp(muy1),lambda,true);
      
      if(y2[j]==0){
        eval += log(1-prob);
      }
      else{
        eval += log(prob) + R::dlnorm(y2[j],muy2,sqrt(sigma2y),true);
      } 
    }
  }
  ll = weight*log(mean(exp(eval)));
  return ll;
}

double loglik_yall(arma::mat y1,arma::mat y2, arma::mat b, arma::mat Z, arma::vec gamma,arma::vec beta, double lambda,double sigma2y,
                   arma::mat Sigmab, arma::vec weight, int m=1000){
  double ll = 0.0;
  int n = y1.n_rows;
  arma::vec out(n);
  for(int i=0;i<n;i++){
    out[i] = loglik_yi(y1.row(i),y2.row(i),b.row(i).t(),Z.row(i),gamma,beta,lambda,sigma2y,Sigmab,weight[i],m);
  }
...
//// [[Rcpp::export]]
double log_qgamma(arma::vec cx, arma::mat vx, arma::mat y1,arma::mat y2, arma::mat b, arma::mat Z, arma::vec gamma,arma::vec beta, double lambda,double sigma2y,
                  arma::mat Sigmab, arma::vec weight, int m){
  double out = 0.0;
  int n = y1.n_rows;
  int k = y1.n_cols;
  
  out += loglik_yall(y1,y2,b,Z,gamma,beta,lambda,sigma2y,Sigmab,weight,m);

    out += dmvnrm_arma(gamma,cx.t(),vx,true);
  return(out);
}

//// [[Rcpp::export]]
arma::vec sample_gamma(arma::vec cx, arma::mat vx, arma::mat y1,arma::mat y2, arma::mat b, arma::mat Z, arma::vec gamma,arma::vec beta, double lambda,double sigma2y,
                       arma::mat Sigmab, arma::vec weight,arma::mat Sigma_1, int m=1000){
  int k = gamma.size();
  int n = y1.n_rows;
  double lacceptprob;
  arma::vec out = gamma;
  ...
  mu1prop = exp(Z*proposal+b.col(0));
  propp = calc_p(mu1prop,lambda);
  
  lacceptprob = log_qgamma(cx,vx,y1,y2,b,Z,proposal,beta,lambda,sigma2y,Sigmab,weight,m)-
    log_qgamma(cx,vx,y1,y2,b,Z,gamma,beta,lambda,sigma2y,Sigmab,weight,m);
...
// double log_ep(double ad, double bd, arma::mat y1,arma::mat y2, arma::mat b, arma::mat Z, arma::vec gamma,arma::vec beta, double lambda,double sigma2y,
//               arma::mat Sigmab, arma::vec weight){
...
//         out += R::dlnorm(y2(i,j),x2(i,j),sqrt(phi),true);
//       }
//     }
//   }
//   return(out);
// }
// 
// double sample_phi(double ad, double bd, arma::mat y1,arma::mat y2, arma::mat b, arma::mat Z, arma::vec gamma,arma::vec beta, double lambda,double sigma2y,
//                   arma::mat Sigmab, arma::vec weight, double propa, double propb){
//   double proposal;
//   double lacceptprob;
//   double out = phi;
//   
//   proposal = R::rgamma(propa,1.0/propb);
//   lacceptprob = log_ep(ad,bd,y1,y2,b,Z,gamma,beta,lambda,sigma2y,Sigmab,weight) - 
//     log_ep(ad,bd,y1,y2,b,Z,gamma,beta,lambda,sigma2y,Sigmab,weight) -
...
double log_ql(arma::mat y1,arma::mat y2, arma::mat b, arma::mat Z, arma::vec gamma,arma::vec beta, double lambda,double sigma2y,
              arma::mat Sigmab, arma::vec weight, double a0l, double b0l, int m){
  double out = 0.0;
  int k = y1.n_cols;
  int n = y1.n_rows;
...
//// [[Rcpp::export]]
double sample_lambda(arma::mat y1,arma::mat y2, arma::mat b, arma::mat Z, arma::vec gamma,arma::vec beta, double lambda,double sigma2y,
                     arma::mat Sigmab, arma::vec weight, double a0l, double b0l,
                     double propl1, double propl2,int m=1000){
  double out = lambda;
  double proposal = R::rbeta(propl1,propl2);
  double lacceptprob;

  
  lacceptprob = log_ql(y1,y2,b,Z,gamma,beta,proposal,sigma2y,Sigmab,weight,a0l,b0l,m) - 
    log_ql(y1,y2,b,Z,gamma,beta,lambda,sigma2y,Sigmab,weight,a0l,b0l,m) -
...
double log_qb(arma::rowvec y1,arma::rowvec y2, arma::vec b, arma::rowvec Z, arma::vec gamma,arma::vec beta, double lambda,double sigma2y,
              arma::mat Sigmab, double weight, int m){
  int n = y1.size();
  int kp = b.size();
  double out = 0.0;
  ...
  out += loglik_yi(y1,y2,b,Z,gamma,beta,lambda,sigma2y,Sigmab,weight,m);
  
  //ll += R::dgamma(x1,eta,1.0/(eta/mux1),true);
  //ll += R::dgamma(x2,theta,1.0/(theta/mux2),true);
  //std::cout << dmvnrm_arma(currentb,zs,Sigmab,true) << " currentb = " << currentb << "Sigmab = " << Sigmab << "\n";
  out += arma::accu(dmvnrm_arma(b,zs,Sigmab,true));
  //}
...
arma::mat sample_b(arma::mat y1,arma::mat y2, arma::mat b, arma::mat Z, arma::vec gamma,arma::vec beta, double lambda,double sigma2y,
                   arma::mat Sigmab, arma::vec weight,arma::cube Sigma_1, int m=1000){
  //std::cout << "a\n";
  ...
  for(int i=0;i<n;i++){
    ...
    propb2 = mvrnormArma(1,b.row(i).t(),pow(2.4,2)*Sigma_1.slice(i)/kp);
    //std::cout << "c\n";
    propb[0] = propb2(0,0);
    propb[1] = propb2(0,1);
    ...
    lacceptprob = log_qb(y1.row(i),y2.row(i),propb,Z.row(i),gamma,beta,lambda,sigma2y,Sigmab,weight[i],m) - 
      log_qb(y1.row(i),y2.row(i),b.row(i).t(),Z.row(i),gamma,beta,lambda,sigma2y,Sigmab,weight[i],m);
    //std::cout << "e\n";
    //std::cout << exp(lacceptprob) << "\n";
    if(log(R::runif(0,1)) < lacceptprob){
      out.row(i) = propb.t();
    }
...
// [[Rcpp::export]]
List mcmc_2part_nci9w(List data, 
                     List init, 
                     List priors, 
                     const int nreps, 
                     const int burn=1000,
  ...
  arma::vec weight    = as<arma::vec>(init["weight"]);
  ...
  for(int i=0;i<nreps;i++){
    ...
    //currentphi = sample_phi(a0delta,b0delta,currentsigma2y,y2,arma::join_rows(currentlmuy,currentlmuy2),propd1,propd2); 
    ...
    currentgamma = sample_gamma(mu0x1,V0x1,y1,y2,currentb,Za,currentgamma,currentbetay,currentlambda,currentsigma2y,
                                currentSigmab,weight,gamma_var,100);
    //std::cout << "6c\n";
    
    currentlmux1 = calc_lmu(Za,currentgamma,currentb.col(0));
    
    currentlambda = sample_lambda(y1,y2,currentb,Za,currentgamma,currentbetay,currentlambda,currentsigma2y,
                                  currentSigmab,weight,a0l,b0l,propl1,propl2,100); 
    ...
    currentb = sample_b(y1,y2,currentb,Za,currentgamma,currentbetay,currentlambda,currentsigma2y,
                        currentSigmab,weight,b_var,100); 
    ...
    if((i>99)&&(i<burn)&&(i%20==0)){
      ...
      if(bvar0count > 0){
        std::cout << "b proposal covariance matrix has variance 0\n";
      }
    }
