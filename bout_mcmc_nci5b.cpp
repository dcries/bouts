  ...
  for(int i=0;i<n;i++){
    for(int j=0;j<k;j++){
      if(y2(i,j)>0){
        ...
        out += R::dlnorm(y2(i,j),mu1(i,j),sqrt(phi),true);
      }
    }
  }
...
arma::vec sample_betay(arma::vec cx, arma::mat vx, double phi, arma::mat mu1, 
  ...
  mu1prop.col(0) = log(pow(Za1*proposal+currentb,2.0));
  mu1prop.col(1) = log(pow(Za2*proposal+currentb,2.0));
...
double log_qb1(arma::rowvec y1, arma::rowvec y2, double muy, arma::vec muy2, double lambdau,
  ...
  for(int i=0;i<n;i++){
    ...
    if(y2[i]==0){
      ...
      ll += log(p0) + R::dlnorm(y2[i],muy2[i],sqrt(phi),true);
...
double log_qb2(arma::rowvec y2, arma::vec muy, double phi,
  ...
  for(int i=0;i<n;i++){
    if(y2[i]==0){
      ...
      ll += log(p0) + R::dlnorm(y2[i],muy[i],sqrt(phi),true);
...
arma::vec sample_b2(arma::mat y2,arma::mat muy2,arma::vec currentb, 
  ...
  for(int i=0;i<n;i++){
    ...
    lacceptprob = log_qb2(y2.row(i),log(pow(muy2prop,2.0)),phi,propb,Sigmab,p0[i]) - 
...
// [[Rcpp::export]]
List mcmc_2part_nci5b(List data, 
                     List init, 
                     List priors, 
                     const int nreps, 
                     const int burn=1000){
  ...
  for(int i=0;i<nreps;i++){
    ...
    currentbetay = sample_betay(mu0y2,V0y2,currentsigma2y,arma::join_rows(log(pow(currentlmuy,2.0)),log(pow(currentlmuy2,2.0))),
    ...
    currentgamma = sample_gamma(mu0x1,V0x1,currentlambda,exp(currentlmux1),
                                y1,y2,arma::join_rows(log(pow(currentlmuy,2.0)),log(pow(currentlmuy2,2.0))),
                                Za,currentgamma,currentp,currentsigma2y,gamma_var,currentb);
    //std::cout << "6c\n";
    
    currentlmux1 = calc_lmu(Za,currentgamma,currentb.col(0));
    
    currentlambda = sample_lambda2(y1,y2,exp(currentlmux1),arma::join_rows(log(pow(currentlmuy,2.0)),log(pow(currentlmuy2,2.0))),
    ...
    currentb.col(0) = sample_b1(y1,y2,exp(currentlmux1),arma::join_rows(log(pow(currentlmuy,2.0)),log(pow(currentlmuy2,2.0))),
                 currentb.col(0),currentsigma2b[0],currentgamma,currentbetay,Za,x1x2y1,x1x2y2,currentlambda,
                 currentsigma2y,b_var.col(0));
    //std::cout << "2\n";
    ...
    currentb.col(1) = sample_b2(y2,arma::join_rows(log(pow(currentlmuy,2.0)),log(pow(currentlmuy2,2.0))),
                 currentb.col(1),currentsigma2b[1],currentbetay,x1x2y1,x1x2y2,currentsigma2y,
