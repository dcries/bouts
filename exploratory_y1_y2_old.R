...
week1weekend1 <- bouts %>% group_by(id) %>% filter(length((id))==2, sum(Weekend)==1, Weekend==0)
week1weekend2 <- bouts %>% group_by(id) %>% filter(length((id))==2, sum(Weekend)==1, Weekend==1)
...
summary(y2exch)
y2exch2 <- lm(totalexcessdiff~nboutsdiff+weekenddiff,data=subset(boutscast,abs(nboutsdiff<11)))
summary(y2exch2)
y2exch3 <- lm(totalexcessdiff~nboutsdiff+weekenddiff,data=subset(boutscast,abs(totalexcessdiff)<2300))
summary(y2exch3)
